...
public class GranularSpectrum {
    ...
    public static double[] granularSpectrum(double[] fullSound, Envelope probEnv, Envelope mixEnv, Synth synth) {
        ...
        for (double time = controlRate; time < (fullSound.length / (double) WaveWriter.SAMPLE_RATE)
            ...
            ArrayList<double[]> peaks = getPeakFreqAmps(fftSample);
            ...
            for (double[] peak : peaks) {
                double peakFreq = peak[0];
                double peakAmp = peak[1];
    ...
    public static double[] granularSpectrum(double[] fullSound, Envelope probEnv, Envelope mixEnv) {
        return granularSpectrum(fullSound, probEnv, mixEnv, new SimpleSynth("resources/bell.wav", 1287));

    }

    public static ArrayList<double[]> getPeakFreqAmps(double[] fftSample) {
        ...
        while (index < spec.size() - 1) {
            ...
            peaks.add(new double[] { peakFreq, peakAmp });
        }
        return peaks;
    }
    ...
    public static double[] granularSpectrum(double[] fullSound) {
        ...
        return granularSpectrum(fullSound, probEnv, mixEnv);
    }

    public static double[] granularSpectrumCrispAttack(double[] fullSound) {
    ...
    public static double[] granularSpectrumCrispAttack(double[] fullSound, Synth synth) {
        Envelope probEnv = new Envelope(new double[] { 0, 0.1, 0.6, 0.8, 0.9, 0.91 },
                new double[] { 1, 0.05, 0.1, 1, 1, 0 });
        Envelope mixEnv = new Envelope(new double[] { 0, 0.7, 0.9 }, new double[] { 0, 0, 1 });
        return granularSpectrum(fullSound, probEnv, mixEnv, synth);
    }

    public static double[] withReverse(double[] sample) {
        int origlen = sample.length;

        double[] sound = granularSpectrum(sample);
        ...
        for (int i = 0; i < fullSound.length; i++) {
            ...
            int framesPastTransition = i - (origlen
